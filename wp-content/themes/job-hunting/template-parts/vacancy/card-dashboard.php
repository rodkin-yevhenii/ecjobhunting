        ...
        <div class="container-fluid">
            <div class="row">
                <div class="d-none d-md-block col-md-2 col-xl-1">
                    <div class="results-image"><img src="<?php
                        echo $vacancy->getEmployer()->getPhoto(); ?>"
                                                    alt="photo"></div>
                </div>
                <div class="col-12 col-md-10 col-xl-11">
                    <div class="container-fluid">
                        <div class="row">
                            <div class="col-12 col-xl-7"><small><?php
                                    echo nicetime(
                                        $vacancy->getDatePosted()
                                    ); ?></small>
                                <h4 class="color-primary"><?php
                                    echo $vacancy->getTitle(); ?> <small><?php
                                        echo sprintf('Job ID: %s', get_the_ID()) ?></small></h4><span
                                        class="text-usual color-secondary mr-5"><?php
                                    echo $vacancy->getCompanyName(); ?></span>
                                <span class="d-block d-md-inline text-usual color-secondary mt-2 mt-md-0"><?php
                                    echo $vacancy->getLocation(); ?></span>
                                <div class="d-flex align-items-start mt-3">
                                    <div class="ys-select ys-select-links mr-3" data-select><span data-select-value>Manage</span>
                                        <ul data-job-id="<?php the_ID();?>">
                                            <li><a class="js-edit-job" data-toggle="modal" data-target="#publish-job__form">Edit job</a></li>
                                            <li><a href="<?php echo $vacancy->getPermalink(); ?>" target="_blank" rel="noreferrer ofollow noopener" class="preview-job">Preview job</a></li>
                                    ...
                                    if ($vacancy->getStatus() !== 'publish') : ?>
                                        <a class="btn btn-grey btn-sm js-publish-job">Publish Job</a>
                                    <?php else: ?>
                                        <a class="btn btn-grey btn-sm js-archive-job">Archive Job</a>
                                    <?php
                                    endif; ?>
                                </div>
                            </div>
